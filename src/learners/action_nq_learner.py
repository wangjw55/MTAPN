...
class ANQLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        if self.args.fix_policy_para or self.args.use_pretain_policy == False:
        ...
        if self.args.optimizer == 'adam':
            ...
            if not self.args.fix_policy_para and self.args.use_pretain_policy:
                self.optimiser2 = Adam(params=self.params_old,  lr=args.lr*args.finetune_lr, weight_decay=getattr(args, "weight_decay", 0))
        else:
            self.optimiser = RMSprop(params=self.params, lr=args.lr, alpha=args.optim_alpha, eps=args.optim_eps)
            if not self.args.fix_policy_para and self.args.use_pretain_policy:
                self.optimiser2 = RMSprop(params=self.params_old, lr=args.lr*args.finetune_lr, alpha=args.optim_alpha, eps=args.optim_eps)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, per_weight=None):
        ...
        self.hidden_index = actions

        for t in range(batch.max_seq_length):
            if t != batch.max_seq_length-1:
                agent_outs = self.mac.forward(batch, t=t, hidden_index=self.hidden_index[:,t])
            else:
        ...
        with th.no_grad():
            ...
            for t in range(batch.max_seq_length):
                if t != batch.max_seq_length-1:
                    target_agent_outs = self.target_mac.forward(batch, t=t, hidden_index=self.hidden_index[:,t])
                else:
